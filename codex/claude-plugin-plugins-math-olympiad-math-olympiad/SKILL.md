...
2. **"Does this prove RH?"** - if your theorem's specialization to ? is a famous
   open problem, you have a gap. Most reliable red flag.
3. **Short proof  to  extract the general lemma** - try 2?2 counterexamples. If
   general form is false, find what's special about THIS instance.
4. **Same gap twice  to  step back** - the case split may be obscuring a unified
...
| ---------------------------------------------------- | ------------------------------------------------------------------------------ | ------------------------- |
| AIME numeric answer                                  | Best-of-N  to  majority vote                                                      | Answer check only         |
...
solve  to  self-improve  to  self-verify  to  correct  to  repeat. This is the Yang-Huang
...
confidently get wrong answers - a pattern that fits n less than or equal to 10 but fails at n=100 is
not a proof.
...
**Rounds**: [how many verify to correct cycles]
...
| Pattern | The check                                                                                                                                                          |
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **#4**  | Does this theorem specialize to a famous object (?, quadratic reciprocity, etc.) and prove something open about it?  to  gap                                          |
| **#18** | Substitute the proof's own intermediate identities into any "remaining gap." Recover the original claim?  to  tautological                                            |
...
| **#5**  | For each invoked theorem: re-check hypotheses FROM SCRATCH. "Continuous on [0,1]" != "continuous on R"                                                              |
...
- 2 say HOLE FOUND  to  refuted, stop (save the remaining 3 calls)
- 4 say HOLDS  to  confirmed, stop (save the 5th)
...
Concrete shape: proving f(n)  less than or equal to  cn for a constrained function f, with a case
split on a prime p dividing f(n). One branch closes by index arguments in
(Z/p^e)\*. The other branch resists - same group structure, but the arithmetic
...
```
NO WEB ACCESS. Do not use WebFetch, WebSearch, or any tool that touches the internet. Do not look up this problem, its solution, or related problems. You are solving this yourself - the only allowed computation is local (Bash/Python for mod-k arithmetic, small-case enumeration n less than or equal to 10, symbolic identity checks). If you invoke a web tool, the proof is void.
```

**Computation bounds in deep mode** (bug #8 lesson): A6's b\_{n+1}=2b_n2+b_n+1
is doubly-exponential; b_99 has ~10^{2^98} digits. Never compute such objects
exactly - work in Z/2^m, or track only v_p(?), or prove the recursion mod the
...
- The instruction: "NO WEB ACCESS - do not look up this problem or its solution.
  Bounded local computation allowed (mod 2^k, small cases n less than or equal to 10, symbolic
  identity checks via Bash/Python only). 60-second computation limit. If n less than or equal to 10
