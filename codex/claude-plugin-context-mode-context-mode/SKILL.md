...
**Everything else  to  `ctx_execute` or `ctx_execute_file`.** Any command that reads, queries, fetches, lists, logs, tests, builds, diffs, inspects, or calls an external service. This includes ALL CLIs (gh, aws, kubectl, docker, terraform, wrangler, fly, heroku, gcloud, etc.) - there are thousands and we cannot list them all.
...
?   ??? Use ctx_fetch_and_index  to  ctx_search
?
??? Using Playwright (navigate, snapshot, console, network)?
?   ??? ALWAYS use filename parameter to save to file, then:
?       browser_snapshot(filename)  to  ctx_index(path) or ctx_execute_file(path)
?       browser_console_messages(filename)  to  ctx_execute_file(path)
?       browser_network_requests(filename)  to  ctx_execute_file(path)
...
?   ?   ??? Save to file via ctx_execute, then ctx_index(path)  to  ctx_search
?   ??? One-shot extraction?
...
| Playwright snapshot | `browser_snapshot(filename)`  to  `ctx_index(path)`  to  `ctx_search` | Save to file, index server-side, query |
| Playwright snapshot (one-shot) | `browser_snapshot(filename)`  to  `ctx_execute_file(path)` | Save to file, extract in sandbox |
| Playwright console/network | `browser_*(filename)`  to  `ctx_execute_file(path)` | Save to file, analyze in sandbox |
| MCP output (already in context) | Use directly | Don't re-index - it's already loaded |
| MCP output (need multi-query) | `ctx_execute` to save  to  `ctx_index(path)`  to  `ctx_search` | Save to file first, index server-side |
...
<sandboxed_data_workflow>
  ...
  <workflow>
    LargeDataTool(filename: "path")  to  mcp__context-mode__ctx_index(path: "path")  to  ctx_search()
  </workflow>
</sandboxed_data_workflow>
...
**When a task involves Playwright snapshots, screenshots, or page inspection, ALWAYS route through file  to  sandbox.**
...
### Workflow A: Snapshot  to  File  to  Index  to  Search (multiple queries)

```
Step 1: browser_snapshot(filename: "/tmp/playwright-snapshot.md")
         to  saves to file, returns ~50B confirmation (NOT 135K tokens)

Step 2: ctx_index(path: "/tmp/playwright-snapshot.md", source: "Playwright snapshot")
         to  reads file SERVER-SIDE, indexes into FTS5, returns ~80B confirmation

Step 3: ctx_search(queries: ["login form email password"], source: "Playwright")
         to  returns only matching chunks (~300B)
```

**Total context: ~430B** instead of 270K tokens. Real 99% savings.

### Workflow B: Snapshot  to  File  to  Execute File (one-shot extraction)

```
Step 1: browser_snapshot(filename: "/tmp/playwright-snapshot.md")
         to  saves to file, returns ~50B confirmation
...
Step 2: ctx_execute_file(path: "/tmp/playwright-snapshot.md", language: "javascript", code: "
        ...
        ")
         to  processes in sandbox, returns ~200B summary
```
...
browser_console_messages(level: "error", filename: "/tmp/console.md")
 to  ctx_execute_file(path: "/tmp/console.md", ...) or ctx_index(path: "/tmp/console.md", ...)

browser_network_requests(includeStatic: false, filename: "/tmp/network.md")
 to  ctx_execute_file(path: "/tmp/network.md", ...) or ctx_index(path: "/tmp/network.md", ...)
```
...
|----------|-------------|----------|
| `browser_snapshot()`  to  raw into context | **135K tokens** | NO |
| `browser_snapshot()`  to  `ctx_index(content: raw)` | **270K tokens** (doubled!) | NO |
| `browser_snapshot(filename)`  to  `ctx_index(path)`  to  `ctx_search` | **~430B** | YES |
| `browser_snapshot(filename)`  to  `ctx_execute_file(path)` | **~250B** | YES |
...
>
> Data flow: **Playwright  to  file  to  server-side read  to  context**. Never: **Playwright  to  context  to  ctx_index(content)  to  context again**.
...
- Using `curl http://api/endpoint` via Bash  to  50KB floods context. Use `ctx_execute` with fetch instead.
- Using `cat large-file.json` via Bash  to  entire file in context. Use `ctx_execute_file` instead.
- Using `gh pr list` via Bash  to  raw JSON in context. Use `ctx_execute` with `--jq` filter instead.
- Piping Bash output through `| head -20`  to  you lose the rest. Use `ctx_execute` to analyze ALL data and print summary.
- Running `npm test` via Bash  to  full test output in context. Use `ctx_execute` to capture and summarize.
- Calling `browser_snapshot()` WITHOUT `filename` parameter  to  135K tokens flood context. **Always** use `browser_snapshot(filename: "/tmp/snap.md")`.
- Calling `browser_console_messages()` or `browser_network_requests()` WITHOUT `filename`  to  entire output floods context. **Always** use the `filename` parameter.
- Passing ANY large data to `ctx_index(content: ...)`  to  data enters context as a parameter. **Always** use `ctx_index(path: ...)` to read server-side. The `content` parameter should only be used for small inline text you're composing yourself.
- Calling an MCP tool (Context7 `query-docs`, GitHub API, etc.) then passing the response to `ctx_index(content: response)`  to  **doubles** context usage. The response is already in context - use it directly or save to file first.
- Ignoring `browser_navigate` auto-snapshot  to  navigation response includes a full page snapshot. Don't rely on it for inspection - call `browser_snapshot(filename)` separately.
- Expecting `ctx_stats` to reset or wipe anything  to  `ctx_stats` is read-only (shows stats only). Use `ctx_purge(confirm: true)` to permanently delete all indexed content.
