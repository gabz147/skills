---
name: claude-plugin-external-plugins-imessage-configure
description: "[Ported from Claude plugin marketplace skill] Check iMessage channel setup and review access policy. Use when the user asks to configure iMessage, asks \"how do I set this up\" or \"who can reach me,\" or wants to know why texts aren't reaching the assistant."
---
...
1. **Full Disk Access** - run `ls ~/Library/Messages/chat.db`. If it fails
   ...
   Settings  to  Privacy & Security  to  Full Disk Access. The server can't read
   chat.db without it."*
...
3. **What next** - end with a concrete next step based on state:
   - FDA not granted  to  the FDA instructions above
   - FDA granted, policy is allowlist  to  *"Text yourself from any device
   ...
   - FDA granted, someone allowed  to  *"Ready. Self-chat works; {N} other
     sender(s) allowed."*
...
3. **"Nobody, just me"**  to  done. The default `allowlist` with an empty list
   is correct. Self-chat bypasses the gate.
4. **"My partner / a friend / a couple people"**  to  ask for each handle
...
5. **Current policy is `pairing`**  to  flag it immediately: *"Your policy is
...
6. **User asks for `pairing`**  to  push back. Explain the auto-reply-to-
