---
name: gsd-quick
description: "[Ported from Claude personal skill] Execute a quick task with GSD guarantees (atomic commits, state tracking) but skip optional agents"
---
...
- Derive display status:
  - SUMMARY.md exists, frontmatter status=complete  to  `complete ?`
  - SUMMARY.md exists, frontmatter status=incomplete OR status missing  to  `incomplete`
  - SUMMARY.md missing, dir created <7 days ago  to  `in-progress`
  - SUMMARY.md missing, dir created  greater than or equal to 7 days ago  to  `abandoned? (>7 days, no summary)`
