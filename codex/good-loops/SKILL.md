...
## The one mental model: reason  to  act  to  observe

```
reason (plan)   to   act (implement)   to   observe (verify)
       ^                                      |
       |______ not done? feedback ___________|
                          |
                     done? stop  to  report to human
```
...
2. **Verification - how does it check?** The agent must have the *tools* to actually verify the
   ...
   - UI / image / game  to  screenshot in a browser, compare visually, play the levels
   - code / script  to  run the test, check flow, lint
   - writing  to  check tone, voice, structure
...
- Sweet spot for long runs is ~35 min to a few hours, or a "chunky loop before bed"  to  wake up
