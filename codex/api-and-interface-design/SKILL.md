...
description: "[Ported from Claude personal skill] Guides stable API and interface design. Use when designing APIs, module boundaries, or any public interface. Use when creating REST or GraphQL endpoints, defining type contracts between modules, or establishing boundaries between frontend and backend."
---
...
// Status code mapping
// 400  to  Client sent invalid data
// 401  to  Not authenticated
// 403  to  Authenticated but not authorized
// 404  to  Resource not found
// 409  to  Conflict (duplicate, version mismatch)
// 422  to  Validation failed (semantically invalid)
// 500  to  Server error (never expose internal details)
```
...
```
GET    /api/tasks               to  List tasks (with query params for filtering)
POST   /api/tasks               to  Create a task
GET    /api/tasks/:id           to  Get a single task
PATCH  /api/tasks/:id           to  Update a task (partial)
DELETE /api/tasks/:id           to  Delete a task

GET    /api/tasks/:id/comments  to  List comments for a task (sub-resource)
POST   /api/tasks/:id/comments  to  Add a comment to a task
```
