...
- The iridescent spectrum is *always* teal  to  violet  to  magenta  to  cyan  to  teal. Never shuffle.
...
- `radialGradient` for body: hi (14%)  to  core (14%)  to  mid (55%)  to  rim (88%)  to  #000.
...
- Breathing animation when `active`: `transform: scale(1)  to  scale(1.02)  to  scale(1)` over 4s.
...
>
  ...
  <span style={{ position: "relative", zIndex: 1 }}>Continue  to </span>
</button>
```
...
- **Entry animation**: `fadeUp` - translateY(12px)  to  0 over 700ms with the emphatic ease. Nested content staggers at 500ms.
...
| Heatmap intensity | Alpha-ramp on `rgba(30,60,255, ?)` with sapphire glow past 0.5 |
