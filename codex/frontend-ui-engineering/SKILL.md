...
description: "[Ported from Claude personal skill] Builds production-quality UIs. Use when building or modifying user-facing interfaces. Use when creating components, implementing layouts, managing state, or when the output needs to look and feel production-quality rather than AI-generated."
---
...
```
Local state (useState)            to  Component-specific UI state
Lifted state                      to  Shared between 2-3 sibling components
Context                           to  Theme, auth, locale (read-heavy, write-rare)
URL state (searchParams)          to  Filters, pagination, shareable UI state
Server state (React Query, SWR)   to  Remote data with caching
Global store (Zustand, Redux)     to  Complex client state shared app-wide
```
...
```
h1  to  Page title (one per page)
h2  to  Section title
h3  to  Subsection title
body  to  Default text
small  to  Secondary/helper text
```
