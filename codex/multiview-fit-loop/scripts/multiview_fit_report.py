...
def main():
    ...
    ap.add_argument('--reference-mode', default='bright', choices=['wire_black','bright','alpha','otsu'])
    ap.add_argument('--render-mode', default='bright', choices=['wire_black','bright','alpha','otsu'])
    ap.add_argument('--required-views', nargs='+', choices=VIEWS)
    args=ap.parse_args()
    report={'schema':'multiview_fit_report.v1','views':{},'summary':{}}
    required = args.required_views or [v for v in VIEWS if getattr(args,f'{v}_template') or getattr(args,f'{v}_render')]
    report['required_views'] = required
    report['validation_scope'] = 'bbox diagnostics only; evaluate silhouette, parts, landmarks, and project thresholds separately'
    all_center=[]; all_size=[]
    for v in VIEWS:
        t=getattr(args,f'{v}_template'); r=getattr(args,f'{v}_render')
        if not (t and r): continue
        rep=compare(t,r,Path(args.overlay_dir)/f'{v}_fit_overlay.png', args.reference_mode, args.render_mode)
        report['views'][v]=rep
        all_center.append(rep.get('pass_bbox_center_1p5pct', False))
        all_size.append(rep.get('pass_bbox_size_5pct', False))
    report['missing_views'] = [v for v in required if v not in report['views']]
    report['summary']['all_required_views_present'] = bool(required) and not report['missing_views']
    ...
    report['summary']['all_views_pass_bbox_diagnostic']=report['summary']['all_required_views_present'] and report['summary']['all_bbox_centers_pass'] and report['summary']['all_bbox_sizes_pass_5pct']
