...
description: Reconstruct Blender models from supplied reference sheets, branding templates, texture atlases, orthographic front/side/back/top views, or mascot/logo art where visual fidelity to the source is more important than a plausible generated object. Use when the user says the model must match a template, wireframe, texture pack, character sheet, mascot sheet, or brand asset exactly; also use after feedback like "does not look like the reference", "fit the texture 1:1", "wrong number of visible parts", or "compare against the template". Uses Blender MCP or the installed blender-headless execution skill plus local Python with Pillow/OpenCV/numpy; pairs with blender-uv-texturing, wireframe-to-3d, blender-modeling, blender-materials, and blender-export.
---
...
- Local analysis Python: `C:/Users/Tarlu/Developer/blender-reference-tools/.venv/Scripts/python.exe`. This isolated environment has OpenCV, NumPy, SciPy, Pillow and scikit-image. In PowerShell invoke a quoted executable with `&`. Script paths are relative to `C:/Users/Tarlu/.codex/skills/reference-to-3d`.
...
```bash
& "C:/Users/Tarlu/Developer/blender-reference-tools/.venv/Scripts/python.exe" "C:/Users/Tarlu/.codex/skills/reference-to-3d/scripts/template_analyzer.py"   --image /path/to/front_reference.png   --out /tmp/front_analysis.json   --mode edges  # use bright_on_dark/dark_on_bright/alpha when cleaner
```
...
```bash
& "C:/Users/Tarlu/Developer/blender-reference-tools/.venv/Scripts/python.exe" "C:/Users/Tarlu/.codex/skills/reference-to-3d/scripts/silhouette_validator.py"   --reference /tmp/reference_mask.png   --render /tmp/render_mask.png   --out /tmp/validation.json
```
...
def create_reference_plane(name, image_path, width, height, y=0.2, alpha=0.35):
    ...
    if hasattr(mat, 'surface_render_method'):
        mat.surface_render_method = 'DITHERED'  # Blender 4.2+/5.x
