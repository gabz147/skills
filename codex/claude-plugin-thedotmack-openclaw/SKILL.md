---
name: claude-plugin-thedotmack-openclaw
description: "[Ported from Claude plugin marketplace skill] Ported Codex skill for openclaw."
---
...
1. Enable Developer Mode in Discord: Settings  to  Advanced  to  Developer Mode
2. Right-click the target channel  to  Copy Channel ID
...
OpenClaw Gateway
  ?
  ??? before_agent_start ??? to  Init session
  ??? before_prompt_build ?? to  Inject context into system prompt
  ??? tool_result_persist ?? to  Record observation
  ??? agent_end ???????????? to  Summarize + Complete session
  ??? gateway_start ???????? to  Reset session tracking + context cache
                    ?
                    ?
         Claude-Mem Worker (localhost:37777)
           ...
           ??? GET  /api/context/inject ?? to  System prompt context
           ??? GET  /stream ????????????? to  SSE  to  Messaging channels
```
...
A background service connects to the worker's SSE stream and forwards `new_observation` events to a configured messaging channel. The connection auto-reconnects with exponential backoff (1s  to  30s max).
