---
name: claude-plugin-plugins-mcp-server-dev-build-mcp-app
...
```html
<script type="module">
  /* ext-apps bundle inlined at build time  to  globalThis.ExtApps */
  /*__EXT_APPS_BUNDLE__*/
...
| `app.ontoolinput = fn` | Host  to  widget | Receive the tool's input args (what Claude passed) |
...
| `app.openLink({url})` | Widget  to  host | Open a URL in a new tab (sandbox blocks `window.open`) |
| `app.getHostContext()` / `app.onhostcontextchanged` | Host  to  widget | Theme, host CSS vars, `containerDimensions`, `displayMode`, `deviceCapabilities` |
| `app.requestDisplayMode({mode})` | Widget  to  host | Ask for `inline` / `pip` / `fullscreen` |
...
**One widget per tool.** Resist the urge to build one mega-widget that does everything. One tool  to  one focused widget  to  one clear result shape. Claude reasons about these far better.
...
**Layout-fork by item count, not by tool count.** If one use case is "show one result in detail" and another is "show many results side-by-side", don't make two tools - make one tool that accepts `items[]`, and let the widget pick a layout: `items.length === 1`  to  detail view, `> 1`  to  carousel. Keeps the server schema simple and lets Claude decide count naturally.
