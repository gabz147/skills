---
name: gsd-thread
description: "[Ported from Claude personal skill] Manage persistent context threads for cross-session work"
---
...
- `"list"` or `""` (empty)  to  LIST mode (show all, default)
- `"list --open"`  to  LIST-OPEN mode (filter to open/in_progress only)
- `"list --resolved"`  to  LIST-RESOLVED mode (resolved only)
- `"close <slug>"`  to  CLOSE mode; extract SLUG = remainder after "close " (sanitize)
- `"status <slug>"`  to  STATUS mode; extract SLUG = remainder after "status " (sanitize)
- matches existing filename (`.planning/threads/{arg}.md` exists)  to  RESUME mode (existing behavior)
- anything else (new description)  to  CREATE mode (existing behavior)
