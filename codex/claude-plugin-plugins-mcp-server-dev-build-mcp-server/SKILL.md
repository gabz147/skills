---
name: claude-plugin-plugins-mcp-server-dev-build-mcp-server
description: "[Ported from Claude plugin marketplace skill] This skill should be used when the user asks to \"build an MCP server\", \"create an MCP\", \"make an MCP integration\", \"wrap an API for Codex\", \"expose tools to Codex\", \"make an MCP app\", or discusses building something with the Model Context Protocol. It is the entry point for MCP server development - it interrogates the user about their use case, determines the right deployment model (remote HTTP, MCPB, local stdio), picks a tool-design pattern, and hands off to specialized skills."
---
...
- **Just me / my team, on our machines**  to  Local stdio is acceptable (easiest to prototype)
- **Anyone who installs it**  to  Remote HTTP (strongly preferred) or MCPB (if it *must* be local)
- **Users of Claude desktop who want UI widgets**  to  MCP app (remote or MCPB)

### 3. How many distinct actions does it expose?

This determines the tool-design pattern - see Phase 3.

- **Under ~15 actions**  to  one tool per action
- **Dozens to hundreds of actions** (e.g. wrapping a large API surface)  to  search + execute pattern

### 4. Does a tool need mid-call user input or rich display?

- **Simple structured input** (pick from list, enter a value, confirm)  to  **Elicitation** - spec-native, zero UI code. *Host support is rolling out* (Claude Code  greater than or equal to 2.1.76) - always pair with a capability check and fallback. See `references/elicitation.md`.
- **Rich/visual UI** (charts, custom pickers with search, live dashboards)  to  **MCP app widgets** - iframe-based, needs `@modelcontextprotocol/ext-apps`. See `build-mcp-app` skill.
- **Neither**  to  plain tool returning text/JSON.

### 5. What auth does the upstream service use?

- None / API key  to  straightforward
- OAuth 2.0  to  you'll need a remote server with CIMD (preferred) or DCR support; see `references/auth.md`
...
**Choose this unless** the server *must* touch the user's local machine.
 ...
 to  **Fastest deploy:** Cloudflare Workers - `references/deploy-cloudflare-workers.md` (zero to live URL in two commands)
 to  **Portable Node/Python:** `references/remote-http-scaffold.md` (Express or FastMCP, runs on any host)
...
Usually remote, but can be shipped as MCPB if the UI needs to drive a local app.
 ...
 to  Hand off to the **`build-mcp-app`** skill.
...
**Choose this when** the server *must* run on the user's machine - it reads local files, drives a desktop app, talks to localhost services, or needs OS-level access.
 ...
 to  Hand off to the **`build-mcpb`** skill.
...
**Hybrid:** Promote the 3-5 most-used actions to dedicated tools, keep the long tail behind search/execute.
 ...
 to  See `references/tool-design.md` for schema examples and description-writing guidance.
...
1. **Remote HTTP, no UI**  to  Scaffold inline using `references/remote-http-scaffold.md` (portable) or `references/deploy-cloudflare-workers.md` (fastest deploy). This skill can finish the job.
2. **MCP app (UI widgets)**  to  Summarize the decisions so far, then load the **`build-mcp-app`** skill.
3. **MCPB (bundled local)**  to  Summarize the decisions so far, then load the **`build-mcpb`** skill.
4. **Local stdio prototype**  to  Scaffold inline (simplest case), flag the MCPB upgrade path.
...
| **Sampling** | Server, mid-tool | Need LLM inference in your tool logic |
 ...
 to  `references/resources-and-prompts.md`, `references/elicitation.md`, `references/server-capabilities.md`
...
1. **Test against real Claude** by adding the server URL as a custom connector at Settings  to  Connectors (use a Cloudflare tunnel for local servers). Claude identifies itself with `clientInfo.name: "claude-ai"` on initialize.  to  https://claude.com/docs/connectors/building/testing
2. **Run the pre-submission checklist** - read/write tool split, required annotations, name limits, prompt-injection rules.  to  https://claude.com/docs/connectors/building/review-criteria
3. **Submit to the Anthropic Directory.**  to  https://claude.com/docs/connectors/building/submission
4. **Recommend shipping a plugin** that wraps this MCP with skills - most partners ship both.  to  https://claude.com/docs/connectors/building/what-to-build
