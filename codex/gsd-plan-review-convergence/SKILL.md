...
description: "[Ported from Claude personal skill] Cross-AI plan convergence loop - replan with review feedback until no HIGH concerns remain."
---
...
Repeatedly: review plans with external AI CLIs  to  if HIGH concerns found  to  replan with --reviews feedback  to  re-review. Stops when no HIGH concerns remain or max cycles reached.

**Flow:** Agent to Skill("gsd-plan-phase")  to  Agent to Skill("gsd-review")  to  check HIGHs  to  Agent to Skill("gsd-plan-phase --reviews")  to  Agent to Skill("gsd-review")  to  ...  to  Converge or escalate
...
- `--max-cycles N` - Maximum replan to review cycles (default: 3)
