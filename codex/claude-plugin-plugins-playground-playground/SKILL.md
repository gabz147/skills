...
- Prompt output is just a value dump  to  write it as a natural instruction
- Too many controls at once  to  group by concern, hide advanced in a collapsible section
- Preview doesn't update instantly  to  every control change must trigger immediate re-render
- No defaults or presets  to  starts empty or broken on load
- External dependencies  to  if CDN is down, playground is dead
- Prompt lacks context  to  include enough that it's actionable without the playground
