...
description: "[Ported from Claude personal skill] Anti-slop frontend skill for landing pages, portfolios, and redesigns. The agent reads the brief, infers the right design direction, and ships interfaces that do not look templated. Real design systems when applicable, audit-first on redesigns, strict pre-flight check."
---
...
LLMs default to cliche?s. Override these defaults proactively. Each rule has a context-aware override path.
...
* **BUTTON CONTRAST CHECK (mandatory, a11y):** Before shipping any button, verify the button text is readable against the button background. White button + white text, `bg-white` CTA with `text-white` label, transparent button against the page background with no border  to  all banned. Audit every CTA: contrast ratio WCAG AA min (4.5:1 for body, 3:1 for large text 18px+). Same rule applies to ghost buttons over photographic backgrounds (use a backdrop, scrim, or stroke).
...
* **NO DUPLICATE CTA INTENT (mandatory):** Two CTAs with the same intent on one page is a Pre-Flight Fail. Examples of same intent: "Get in touch" + "Contact us" + "Let's talk" + "Start a project" + "Start something" + "Reach out" = all "contact" intent  to  pick ONE label and use it everywhere on the page (nav, hero, footer). Same for "Try free" + "Get started" + "Sign up free" (all "signup" intent) and "View work" + "See selected work" + "Browse projects" (all "portfolio" intent). One label per intent.
* **FORM CONTRAST CHECK (mandatory, a11y):** Form inputs, placeholder text, focus rings, helper text, and error text all pass WCAG AA contrast against the section background. Light placeholders on a near-white form, white form on white page section, form labels grayer than 4.5:1 contrast  to  all banned. Audit every form before shipping.
...
* **BENTO CELL COUNT RULE (mandatory):** A bento grid has EXACTLY as many cells as you have content for. 3 items  to  3 cells (1+2 split, or 2+1, or asymmetric trio). 5 items  to  5 cells (2+3, 3+2, hero+4, etc.). If your grid has an empty cell in the middle or at the end, you planned wrong. Re-shape the grid; do not paste a blank tile.
...
* **Default content shape per section:** short headline ( less than or equal to  8 words) + short sub-paragraph ( less than or equal to  25 words) + one visual asset OR one CTA. Anything more must be justified by the section's job.
...
* **Max 3 lines** of quote body. Never 6. If the original quote is longer  to  cut it. A landing-page quote is a snippet, not the full review.
...
* **NO generic names.** "John Doe", "Sarah Chan", "Jack Su"  to  use creative, realistic, locale-appropriate names.
* **NO generic avatars.** No SVG "egg" or Lucide user icons  to  use believable photo placeholders or specific styling.
* **NO fake-perfect numbers.** Avoid `99.99%`, `50%`, `1234567`. Use organic, messy data (`47.2%`, `+1 (312) 847-1928`).
* **NO startup-slop brand names.** "Acme", "Nexus", "SmartFlow", "Cloudly"  to  invent contextual, premium names that sound real.
* **NO filler verbs.** "Elevate", "Seamless", "Unleash", "Next-Gen", "Revolutionize"  to  concrete verbs only.
...
* **NO vertical rotated text** ("INDEX OF WORK, 2018 - 2026" rotated 90?). Agency-portfolio cliche?. Use it only when the brief is explicitly agency / Awwwards / experimental AND it serves a real composition purpose.
...
* **Horizontal Scroll Hijack** - Vertical scroll  to  horizontal pan.
...
* IA, content, and SEO sound  to  **targeted evolution** (Levers 1-4). ~70% of value at ~40% of risk.
* Visual debt is structural (broken IA, no design system, broken mobile)  to  **full redesign** with strict content preservation.
* Brand itself is changing  to  **greenfield**.
...
- [ ] **Hero fits the viewport**: headline  less than or equal to  2 lines, subtext  less than or equal to  20 words AND  less than or equal to  4 lines, CTA visible without scroll, font scale planned around image?
...
- [ ] **EYEBROW COUNT (mechanical)**: count instances of `uppercase tracking` micro-labels above section headlines across all components. Count  less than or equal to  ceil(sectionCount / 3)? Hero counts as 1.
...
- [ ] **Navigation on ONE line** at desktop, height  less than or equal to  80px?
- [ ] **Section-Layout-Repetition** check: no two sections share the same layout family (at least 4 different families across 8 sections)?
- [ ] **Bento has rhythm AND exact cell count** (N items  to  N cells, no empty cells in middle or at end)?
...
- [ ] **Content density** sane: no 20-row data tables, no fake-precise specs without justification,  less than or equal to  25-word sub-paragraphs by default?
- [ ] **Quotes  less than or equal to  3 lines** of body, attribution clean (no em-dash)?
...
- Apple Human Interface Guidelines  to  Materials
- Apple Developer Documentation  to  Liquid Glass
- Apple Developer Documentation  to  Adopting Liquid Glass
- SwiftUI  to  Material
