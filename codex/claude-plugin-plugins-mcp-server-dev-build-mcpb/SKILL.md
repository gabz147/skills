---
name: claude-plugin-plugins-mcp-server-dev-build-mcpb
...
??? manifest.json            from  identity, entry point, config schema, compatibility
??? server/                  from  your MCP server code
?   ??? index.js
?   ??? node_modules/        from  bundled dependencies (or vendored)
??? icon.png
```
