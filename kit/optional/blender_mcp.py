...
@persistent
def _reference_mcp_undo_post(scene, depsgraph=None):
    _edit_recorder.record_undo("undo")


@persistent
def _reference_mcp_redo_post(scene, depsgraph=None):
    _edit_recorder.record_undo("redo")


@persistent
def _reference_mcp_depsgraph_post(scene, depsgraph=None):
...
def _register_edit_capture_handlers():
    ...
    handlers = [
        (bpy.app.handlers.undo_post, _reference_mcp_undo_post),
        (bpy.app.handlers.redo_post, _reference_mcp_redo_post),
        (bpy.app.handlers.depsgraph_update_post, _reference_mcp_depsgraph_post),
    ]
...
def sync_edit_capture_handlers():
    ...
    try:
        server_running = bool(
            getattr(bpy.types, "reference_mcp_server", None)
            and bpy.types.reference_mcp_server.running
        )
    except Exception:
...
def get_reference_mcp_addon_preferences(context=None):
...
class BlenderMCPServer:
    ...
    def _get_config_value(self, scene_attr, pref_attr=None, env_var=None):
        ...
        prefs = get_reference_mcp_addon_preferences()
    ...
    def _get_hyper3d_api_key(self):
        ...
        scene_value = getattr(bpy.context.scene, "reference_mcp_hyper3d_api_key", "")
        if scene_value == RODIN_FREE_TRIAL_KEY:
            return scene_value
        return self._get_config_value(
            "reference_mcp_hyper3d_api_key",
            "hyper3d_api_key",
            "REFERENCE_MCP_HYPER3D_API_KEY",
        )
    ...
    def _get_sketchfab_api_key(self):
        return self._get_config_value(
            "reference_mcp_sketchfab_api_key",
            "sketchfab_api_key",
            "REFERENCE_MCP_SKETCHFAB_API_KEY",
        )
    ...
    def _get_polypizza_api_key(self):
        return self._get_config_value(
            "reference_mcp_polypizza_api_key",
            "polypizza_api_key",
            "REFERENCE_MCP_POLYPIZZA_API_KEY",
        )
    ...
    def _get_hunyuan3d_secret_id(self):
        return self._get_config_value(
            "reference_mcp_hunyuan3d_secret_id",
            "hunyuan3d_secret_id",
            "REFERENCE_MCP_HUNYUAN3D_SECRET_ID",
        )
    ...
    def _get_hunyuan3d_secret_key(self):
        return self._get_config_value(
            "reference_mcp_hunyuan3d_secret_key",
    ...
    def _get_hunyuan3d_api_url(self):
        return self._get_config_value(
            "reference_mcp_hunyuan3d_api_url",
            "hunyuan3d_api_url",
            "REFERENCE_MCP_HUNYUAN3D_API_URL",
    ...
    def start(self):
        ...
        try:
            ...
            # Windows REUSEADDR allows multiple Blender processes on one port,
            # making command routing ambiguous. Reserve this listener exclusively.
            if os.name == 'nt':
    ...
    def _execute_command_internal(self, command):
        ...
        if bpy.context.scene.reference_mcp_use_polyhaven:
            polyhaven_handlers = {
        ...
        if bpy.context.scene.reference_mcp_use_hyper3d:
            polyhaven_handlers = {
        ...
        if bpy.context.scene.reference_mcp_use_sketchfab:
            sketchfab_handlers = {
        ...
        if bpy.context.scene.reference_mcp_use_polypizza:
            polypizza_handlers = {
        ...
        if bpy.context.scene.reference_mcp_use_hunyuan3d:
            hunyuan_handlers = {
    ...
    def get_polyhaven_status(self):
        ...
        enabled = bpy.context.scene.reference_mcp_use_polyhaven
        if enabled:
    ...
    def get_hyper3d_status(self):
        ...
        enabled = bpy.context.scene.reference_mcp_use_hyper3d
        ...
        if enabled:
            ...
            mode = bpy.context.scene.reference_mcp_hyper3d_mode
    ...
    def create_rodin_job(self, *args, **kwargs):
        match bpy.context.scene.reference_mcp_hyper3d_mode:
            case "MAIN_SITE":
    ...
    def get_sketchfab_status(self):
        ...
        enabled = bpy.context.scene.reference_mcp_use_sketchfab
    ...
    def get_polypizza_status(self):
        ...
        enabled = bpy.context.scene.reference_mcp_use_polypizza
    ...
    def get_hunyuan3d_status(self):
        ...
        enabled = bpy.context.scene.reference_mcp_use_hunyuan3d
        hunyuan3d_mode = bpy.context.scene.reference_mcp_hunyuan3d_mode
    ...
    def create_hunyuan_job(self, *args, **kwargs):
        match bpy.context.scene.reference_mcp_hunyuan3d_mode:
            case "OFFICIAL_API":
    ...
    def create_hunyuan_job_local_site(
        ...
        try:
            ...
            octree_resolution = bpy.context.scene.reference_mcp_hunyuan3d_octree_resolution
            num_inference_steps = bpy.context.scene.reference_mcp_hunyuan3d_num_inference_steps
            guidance_scale = bpy.context.scene.reference_mcp_hunyuan3d_guidance_scale
            texture = bpy.context.scene.reference_mcp_hunyuan3d_texture
...
class REFERENCE_MCP_AddonPreferences(bpy.types.AddonPreferences):
    bl_idname = __name__
    ...
    def draw(self, context):
        ...
        box.separator()
        row = box.row()
        row.operator("reference_mcp.open_terms", text="View Terms and Conditions", icon='TEXT')
...
# Blender UI Panel
class REFERENCE_MCP_PT_Panel(bpy.types.Panel):
    bl_label = "MCP for Blender"
    bl_idname = "REFERENCE_MCP_PT_Panel"
    ...
    def draw(self, context):
        layout = self.layout
        scene = context.scene
        prefs = get_reference_mcp_addon_preferences(context)

        # Connection
        box = layout.box()
        col = box.column()
        if scene.reference_mcp_server_running:
            col.label(text=f"Connected on port {scene.reference_mcp_port}", icon='CHECKMARK')
            col.operator("reference_mcp.stop_server", text="Disconnect", icon='X')
        else:
            col.label(text="Not connected", icon='RADIOBUT_OFF')
            col.prop(scene, "reference_mcp_port")
            col.operator("reference_mcp.start_server", text="Connect to MCP server", icon='PLAY')
        ...
        self._integration_header(
            layout, scene, "reference_mcp_use_polyhaven", "Poly Haven", 'WORLD')

        sub = self._integration_header(
            layout, scene, "reference_mcp_use_sketchfab", "Sketchfab", 'MESH_MONKEY')
        if sub:
            col = sub.column(align=True)
            if prefs:
                col.prop(prefs, "sketchfab_api_key", text="API Key")
            else:
                col.prop(scene, "reference_mcp_sketchfab_api_key", text="API Key")

        sub = self._integration_header(
            layout, scene, "reference_mcp_use_polypizza", "Poly Pizza", 'MESH_ICOSPHERE')
        if sub:
            col = sub.column(align=True)
            if prefs:
                col.prop(prefs, "polypizza_api_key", text="API Key")
            else:
                col.prop(scene, "reference_mcp_polypizza_api_key", text="API Key")
        ...
        sub = self._integration_header(
            layout, scene, "reference_mcp_use_hyper3d", "Hyper3D Rodin", 'MESH_UVSPHERE')
        if sub:
            col = sub.column(align=True)
            col.prop(scene, "reference_mcp_hyper3d_mode", text="Mode")
            if prefs:
                col.prop(prefs, "hyper3d_api_key", text="API Key")
            else:
                col.prop(scene, "reference_mcp_hyper3d_api_key", text="API Key")
            sub.operator("reference_mcp.set_hyper3d_free_trial_api_key",
                         text="Set Free Trial API Key", icon='KEYINGSET')

        sub = self._integration_header(
            layout, scene, "reference_mcp_use_hunyuan3d", "Tencent Hunyuan 3D", 'MESH_CUBE')
        if sub:
            col = sub.column(align=True)
            col.prop(scene, "reference_mcp_hunyuan3d_mode", text="Mode")
            if scene.reference_mcp_hunyuan3d_mode == 'OFFICIAL_API':
                if prefs:
                    col.prop(prefs, "hunyuan3d_secret_id", text="SecretId")
                    col.prop(prefs, "hunyuan3d_secret_key", text="SecretKey")
                else:
                    col.prop(scene, "reference_mcp_hunyuan3d_secret_id", text="SecretId")
                    col.prop(scene, "reference_mcp_hunyuan3d_secret_key", text="SecretKey")
            if scene.reference_mcp_hunyuan3d_mode == 'LOCAL_API':
                if prefs:
                    col.prop(prefs, "hunyuan3d_api_url", text="API URL")
                else:
                    col.prop(scene, "reference_mcp_hunyuan3d_api_url", text="API URL")
                col.separator()
                col.prop(scene, "reference_mcp_hunyuan3d_octree_resolution", text="Octree Resolution")
                col.prop(scene, "reference_mcp_hunyuan3d_num_inference_steps", text="Inference Steps")
                col.prop(scene, "reference_mcp_hunyuan3d_guidance_scale", text="Guidance Scale")
                col.prop(scene, "reference_mcp_hunyuan3d_texture", text="Generate Texture")
...
class REFERENCE_MCP_OT_SetFreeTrialHyper3DAPIKey(bpy.types.Operator):
    bl_idname = "reference_mcp.set_hyper3d_free_trial_api_key"
    ...
    def execute(self, context):
        ...
        context.scene.reference_mcp_hyper3d_api_key = RODIN_FREE_TRIAL_KEY
        context.scene.reference_mcp_hyper3d_mode = 'MAIN_SITE'
        self.report({'INFO'}, "API Key set successfully!")
        return {'FINISHED'}

# Operator to start the server
class REFERENCE_MCP_OT_StartServer(bpy.types.Operator):
    bl_idname = "reference_mcp.start_server"
    ...
    def execute(self, context):
        ...
        if not hasattr(bpy.types, "reference_mcp_server") or not bpy.types.reference_mcp_server:
            bpy.types.reference_mcp_server = BlenderMCPServer(port=scene.reference_mcp_port)

        # Start the server
        bpy.types.reference_mcp_server.start()
        scene.reference_mcp_server_running = bpy.types.reference_mcp_server.running

        return {'FINISHED'}

# Operator to stop the server
class REFERENCE_MCP_OT_StopServer(bpy.types.Operator):
    bl_idname = "reference_mcp.stop_server"
    bl_label = "Stop MCP Connection"
    bl_description = "Stop MCP Connection"

    def execute(self, context):
        scene = context.scene

        # Stop the server if it exists
        if hasattr(bpy.types, "reference_mcp_server") and bpy.types.reference_mcp_server:
            bpy.types.reference_mcp_server.stop()
            del bpy.types.reference_mcp_server

        scene.reference_mcp_server_running = False

        return {'FINISHED'}

# Operator to open Terms and Conditions
class REFERENCE_MCP_OT_OpenTerms(bpy.types.Operator):
    bl_idname = "reference_mcp.open_terms"
...
def register():
    bpy.types.Scene.reference_mcp_port = IntProperty(
        name="Port",
    ...
    bpy.types.Scene.reference_mcp_server_running = bpy.props.BoolProperty(
        name="Server Running",
        default=False
    )

    bpy.types.Scene.reference_mcp_auto_start_server = bpy.props.BoolProperty(
        name="Auto-Start Server",
        description="Automatically start the MCP server when Blender loads",
        default=True
    )

    bpy.types.Scene.reference_mcp_use_polyhaven = bpy.props.BoolProperty(
        name="Use Poly Haven",
        description="Enable Poly Haven asset integration",
        default=False
    )

    bpy.types.Scene.reference_mcp_use_hyper3d = bpy.props.BoolProperty(
        name="Use Hyper3D Rodin",
        description="Enable Hyper3D Rodin generatino integration",
        default=False
    )

    bpy.types.Scene.reference_mcp_hyper3d_mode = bpy.props.EnumProperty(
        name="Rodin Mode",
    ...
    bpy.types.Scene.reference_mcp_hyper3d_api_key = bpy.props.StringProperty(
        name="Hyper3D API Key",
        subtype="PASSWORD",
    ...
    bpy.types.Scene.reference_mcp_use_hunyuan3d = bpy.props.BoolProperty(
        name="Use Hunyuan 3D",
        description="Enable Hunyuan asset integration",
        default=False
    )

    bpy.types.Scene.reference_mcp_hunyuan3d_mode = bpy.props.EnumProperty(
        name="Hunyuan3D Mode",
    ...
    bpy.types.Scene.reference_mcp_hunyuan3d_secret_id = bpy.props.StringProperty(
        name="Hunyuan 3D SecretId",
        description="SecretId provided by Hunyuan 3D",
        default=""
    )

    bpy.types.Scene.reference_mcp_hunyuan3d_secret_key = bpy.props.StringProperty(
    ...
    bpy.types.Scene.reference_mcp_hunyuan3d_api_url = bpy.props.StringProperty(
        name="API URL",
        description="URL of the Hunyuan 3D API service",
        default="http://localhost:8081"
    )

    bpy.types.Scene.reference_mcp_hunyuan3d_octree_resolution = bpy.props.IntProperty(
    ...
    bpy.types.Scene.reference_mcp_hunyuan3d_num_inference_steps = bpy.props.IntProperty(
    ...
    bpy.types.Scene.reference_mcp_hunyuan3d_guidance_scale = bpy.props.FloatProperty(
        name="Guidance Scale",
    ...
    bpy.types.Scene.reference_mcp_hunyuan3d_texture = bpy.props.BoolProperty(
        name="Generate Texture",
        description="Whether to generate texture for the 3D model",
        default=False,
    )
    
    bpy.types.Scene.reference_mcp_use_sketchfab = bpy.props.BoolProperty(
        name="Use Sketchfab",
        description="Enable Sketchfab asset integration",
        default=False
    )

    bpy.types.Scene.reference_mcp_sketchfab_api_key = bpy.props.StringProperty(
        name="Sketchfab API Key",
        subtype="PASSWORD",
    ...
    bpy.types.Scene.reference_mcp_use_polypizza = bpy.props.BoolProperty(
        name="Use Poly Pizza",
        description="Enable Poly Pizza asset integration",
        default=False
    )

    bpy.types.Scene.reference_mcp_polypizza_api_key = bpy.props.StringProperty(
        name="Poly Pizza API Key",
        subtype="PASSWORD",
    ...
    bpy.utils.register_class(REFERENCE_MCP_AddonPreferences)

    bpy.utils.register_class(REFERENCE_MCP_PT_Panel)
    bpy.utils.register_class(REFERENCE_MCP_OT_SetFreeTrialHyper3DAPIKey)
    bpy.utils.register_class(REFERENCE_MCP_OT_StartServer)
    bpy.utils.register_class(REFERENCE_MCP_OT_StopServer)
    bpy.utils.register_class(REFERENCE_MCP_OT_OpenTerms)

    # Auto-start the server so the MCP client can connect without manual UI interaction
    scene = getattr(bpy.context, 'scene', None)
    if scene is not None:
        port = scene.reference_mcp_port
        auto_start = scene.reference_mcp_auto_start_server
    else:
        port = 9876
        auto_start = True

    if auto_start and (not hasattr(bpy.types, "reference_mcp_server") or not bpy.types.reference_mcp_server):
        bpy.types.reference_mcp_server = BlenderMCPServer(port=port)
    if auto_start and not bpy.types.reference_mcp_server.running:
        bpy.types.reference_mcp_server.start()
        try:
            bpy.context.scene.reference_mcp_server_running = bpy.types.reference_mcp_server.running
...
def unregister():
    ...
    bpy.utils.unregister_class(REFERENCE_MCP_PT_Panel)
    bpy.utils.unregister_class(REFERENCE_MCP_OT_SetFreeTrialHyper3DAPIKey)
    bpy.utils.unregister_class(REFERENCE_MCP_OT_StartServer)
    bpy.utils.unregister_class(REFERENCE_MCP_OT_StopServer)
    bpy.utils.unregister_class(REFERENCE_MCP_OT_OpenTerms)
    bpy.utils.unregister_class(REFERENCE_MCP_AddonPreferences)

    del bpy.types.Scene.reference_mcp_port
    del bpy.types.Scene.reference_mcp_server_running
    del bpy.types.Scene.reference_mcp_auto_start_server
    del bpy.types.Scene.reference_mcp_use_polyhaven
    del bpy.types.Scene.reference_mcp_use_hyper3d
    del bpy.types.Scene.reference_mcp_hyper3d_mode
    del bpy.types.Scene.reference_mcp_hyper3d_api_key
    del bpy.types.Scene.reference_mcp_use_sketchfab
    del bpy.types.Scene.reference_mcp_sketchfab_api_key
    del bpy.types.Scene.reference_mcp_use_polypizza
    del bpy.types.Scene.reference_mcp_polypizza_api_key
    del bpy.types.Scene.reference_mcp_use_hunyuan3d
    del bpy.types.Scene.reference_mcp_hunyuan3d_mode
    del bpy.types.Scene.reference_mcp_hunyuan3d_secret_id
    del bpy.types.Scene.reference_mcp_hunyuan3d_secret_key
    del bpy.types.Scene.reference_mcp_hunyuan3d_api_url
    del bpy.types.Scene.reference_mcp_hunyuan3d_octree_resolution
    del bpy.types.Scene.reference_mcp_hunyuan3d_num_inference_steps
    del bpy.types.Scene.reference_mcp_hunyuan3d_guidance_scale
    del bpy.types.Scene.reference_mcp_hunyuan3d_texture
