...
Use the shared controller for live checkpoints. A Stop-hook request is not completion; only the source-bound receipt verifies capture. Resolve the runtime model from trusted metadata or the current transcript's `message.model`.
