...
/**
 * Resolve the current git branch for the given directory.
 * Uses HEAD file directly to avoid spawning git when possible (no lock needed).
 * Falls back to `git symbolic-ref` for worktrees. Silent fail → null.
 */
    ...
    for (let i = 0; i < 15; i++) {
      ...
      if (fs.existsSync(gitPath)) {
        ...
        if (stat.isDirectory()) {
          ...
          const realGit = path.resolve(current, match[1]);
          headFile = path.join(realGit, 'HEAD');
        }
...
function runStatusline() {
  ...
  try {
    ...
    const dirStr = branch
      ? `\x1b[2m${dirname}\x1b[0m \x1b[36m${branch}\x1b[0m`
    ...
    const now = new Date();
    const timeStr = `\x1b[2m${now.getHours().toString().padStart(2,'0')}:${now.getMinutes().toString().padStart(2,'0')}\x1b[0m`;

    // Session cost — defensive: Claude Code may expose this as cost.total_cost_usd
    // or cost_usd at the top level; only shown when the field is actually present.
    const costUsd = data.cost?.total_cost_usd ?? data.cost_usd ?? null;
    const costStr = costUsd != null
      ? ` \x1b[2m$${costUsd.toFixed(4)}\x1b[0m`
      : '';

    if (middle) {
      process.stdout.write(`${gsdUpdate}\x1b[2m${model}\x1b[0m │ ${middle} │ ${dirStr}${ctx}${costStr} │ ${timeStr}${lastCmdSuffix}`);
    } else {
      process.stdout.write(`${gsdUpdate}\x1b[2m${model}\x1b[0m │ ${dirStr}${ctx}${costStr} │ ${timeStr}${lastCmdSuffix}`);
    }
  } catch (e) {
...
function renderStatusline(data) {
  ...
  if (middle) {
    return `\x1b[2m${model}\x1b[0m │ ${middle} │ ${dirStr}${lastCmdSuffix}`;
  }
  return `\x1b[2m${model}\x1b[0m │ ${dirStr}${lastCmdSuffix}`;
}
