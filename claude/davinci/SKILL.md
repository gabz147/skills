...
- `alive: true` → daemon is running, proceed.
- `alive: false` or timeout → daemon is not running. Tell the user: *"open Resolve, then click `Workspace → Scripts → cli_anything_daemon`"*. You CANNOT start it yourself — it must be triggered from inside Resolve's UI. Wait for the user to confirm before continuing.
...
For multi-step manipulations like "import clip, rotate, scale, apply LUT, render" — write a Python helper script and either drop it in `%APPDATA%\Blackmagic Design\DaVinci Resolve\Support\Fusion\Scripts\Utility\` to run from Workspace → Scripts, OR send it as a series of bridge calls. The bridge supports any dotted path the daemon can resolve, but multi-statement logic (loops, conditionals) is cleaner as an in-Resolve script.
