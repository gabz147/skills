---
name: mercura
description: Work on the Mercura V2 project — "Mercura — Aurelia's Academy", a React/JSX single-page app located at C:\Users\Tarlu\Desktop\model\Mercura V2. Use when the user types /mercura, says "open mercura", "mercura v2", or otherwise references this project. Triggers should treat the project root as that folder.
---
...
### Proactive behavior
- **Page-aware comments** — detect active route, fire `aureliaThink()` with a context-relevant quip. Already fully scaffolded — just needs a lookup table of route → comment and a `hashchange` listener.
...
- **Global drag-and-drop** — drag any image onto any page → Aurelia reacts via vision, no need to go to Generate.
