---
name: claude-design
description: Front-end design and prototyping in HTML/React, mirroring the working style of claude.ai/design (Mercura). Use when building UI mockups, wireframes, interactive prototypes, single-file HTML artifacts, slide decks, or animated/video-style HTML. Covers React+Babel inline-JSX setup with pinned versions, file/asset conventions, scope-collision rules, when to ask clarifying questions, and content/typography defaults. Skip for backend work, CLI tools, or non-UI tasks.
---
...
- **Significant revisions get versioned copies:** `My Design.html` → `My Design v2.html`. Preserve the old.
